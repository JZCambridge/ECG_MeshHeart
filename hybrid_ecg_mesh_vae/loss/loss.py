...
def VAECELoss(v_pre, v_gt, f, logvar, mu, beta=1e-2, lambd=1, lambd_s=1, loss='cham_smooth'):#1e-2
    ...
    # ============================================================================
    # SMOOTHNESS LOSS COMPUTATION (OPTIMIZED - VECTORIZED)
    # ============================================================================
    # Original implementation (SLOW - 50 iterations per batch):
    # Creates 50 separate Meshes objects, each with overhead from PyTorch3D
    # loss_smooth_all = 0
    # if v_pre.shape[0] > 1:
    ...
    # Optimized implementation (FAST - single batched operation):
    # Create ONE batched Meshes object with all frames: [B*seq_len, V, 3]
    # This reduces PyTorch3D overhead by 50x
    if v_pre.shape[0] > 1:
        v_pre = v_pre.unsqueeze(0)
        v_gt = v_gt.unsqueeze(0)
        f = f.unsqueeze(0)

    # Reshape to batch all frames together: [B, seq_len, V, 3] -> [B*seq_len, V, 3]
    v_pre_batched = v_pre.reshape(-1, v_pre.shape[-2], v_pre.shape[-1])  # [B*seq_len, V, 3]
    f_batched = f.reshape(-1, f.shape[-2], f.shape[-1])  # [B*seq_len, F, 3]

    # Create single batched Meshes object (1 construction instead of seq_len constructions)
    mesh_pre_batched = Meshes(verts=v_pre_batched, faces=f_batched)

    # Compute smoothness on entire batch, then average
