...
# -*-coding:utf-8 -*-
"""
ECG Phenotype LASSO Regression Training Script - SEX-STRATIFIED VERSION
Trains LASSO classifier to predict disease status from 16 ECG morphology features
...
Adapted from main_phenotype_lasso.py for sex stratification
"""
...
def setup_logging(checkpoint_dir: str):
    ...
    logger.info("="*80)
    logger.info("ECG Phenotype LASSO Regression Training - SEX-STRATIFIED")
    logger.info("="*80)
...
def train_lasso_model_stratified(args):
    """
    Main function for training sex-stratified LASSO classifier.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="ECG Phenotype LASSO Regression Training - Sex-Stratified"
    )
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="jzheng12/Codes/ECG_MeshHeart/output/ecg_phenotype_lasso_stratified/",
    ...
    args.checkpoint_dir = (args.checkpoint_dir + args.output_prefix + sex_label + '_' +
                          args.label_column + '_' + args.threshold_direction +
                          str(args.threshold) + '_' + dt_string)

    train_lasso_model_stratified(args)
