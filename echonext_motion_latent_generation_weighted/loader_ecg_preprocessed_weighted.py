...
class ECGMotionDatasetPreprocessed(Dataset):
    ...
    def __init__(
        ...
        is_train: bool = True,
        load_clinical_cols: bool = True,
        clinical_cols: Optional[list] = None,
    ):
        """
        ...
        Args:
            ...
            load_clinical_cols: Whether to load clinical columns for weighted loss
            clinical_cols: List of clinical column names to load
        """
        self.csv_path = csv_path
        self.preprocessed_ecg_path = preprocessed_ecg_path
        self.ecg_phenotypes_path = ecg_phenotypes_path
        self.is_train = is_train
        self.load_clinical_cols = load_clinical_cols
        self.clinical_cols = clinical_cols or ['lv_ef_fr0_percent', 'wt_max', 'LVEDVi_ml/m2']
        ...
        # Define column names
        self.motion_cols = [f'mesh_embed_{i}' for i in range(1, 513)]
        ...
        # Load clinical columns if enabled
        self.clinical_features = None
        if self.load_clinical_cols:
            logger.info(f"Loading clinical columns: {self.clinical_cols}")
            missing_clinical_cols = []
            for col in self.clinical_cols:
                if col not in self.df.columns:
                    missing_clinical_cols.append(col)

            if missing_clinical_cols:
                logger.warning(f"Clinical columns not found: {missing_clinical_cols}")
                logger.warning("Disabling clinical column loading")
                self.load_clinical_cols = False
            else:
                self.clinical_features = self.df[self.clinical_cols].values.astype(np.float32)
                # Handle missing values (fill with median or 0)
                self.clinical_features = np.nan_to_num(self.clinical_features, nan=0.0)
                logger.info(f"✅ Loaded clinical features shape: {self.clinical_features.shape}")
    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dictionary with:
                ...
                motion_latent: [512] tensor
                eid: int
                clinical: [3] tensor (if load_clinical_cols=True)
        """
        eid = self.eids[idx]
        ...
        item = {
            ...
            "motion_latent": torch.FloatTensor(self.motion_embeddings[idx]),  # [512]
            "eid": eid
        }

        # Add clinical features if loaded
        if self.load_clinical_cols and self.clinical_features is not None:
            item["clinical"] = torch.FloatTensor(self.clinical_features[idx])  # [3]
...
class MotionDataModulePreprocessed:
    ...
    def __init__(
        ...
        Args:
            ...
            test_csv_path: Optional path to test CSV
            load_clinical_cols: Whether to load clinical columns
        """
    ...
    def setup(self, stage: str = "fit"):
        ...
        if stage == "fit" or stage is None:
            ...
            self.train_dataset = ECGMotionDatasetPreprocessed(
                ...
                is_train=True,
                load_clinical_cols=self.load_clinical_cols,
            )
            ...
            # Setup test dataset if path provided
            if self.test_csv_path is not None:
                print("\n=== Setting up test dataset ===")
                self.test_dataset = ECGMotionDatasetPreprocessed(
